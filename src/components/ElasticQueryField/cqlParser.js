...
const cqlParser = (intl) => {
  ...
  CQLParser.prototype = {
    parse(query) {
      if (!query) throw new Error(intl.formatMessage({ id: 'ui-inventory-es.parser.emptyQuery' }));
      this.qs = query;
      this.ql = this.qs.length;
      this.qi = 0;
      this._move();
      this.tree = this._parseQuery(this.scf, this.scr, []);
      if (this.look !== '') throw new Error(intl.formatMessage({ id: 'ui-inventory-es.parser.eof' }));
    },
    toString() {
    ...
    _parseModifiers() {
      ...
      while (this.look === '/') {
        ...
        if (this.look !== 's' && this.look !== 'q') {
          throw new Error(intl.formatMessage({ id: 'ui-inventory-es.parser.invalidModifier' }));
        }
        ...
        if (this.look.length > 0
          ...
          this._move();
          if (this.look !== 's' && this.look !== 'q') {
            throw new Error(intl.formatMessage({ id: 'ui-inventory-es.parser.invalidRelation' }));
          }
    ...
    _parseSearchClause(field, relation, modifiers) {
      if (this.look === '(') {
        ...
        this._move();
        if (this.look !== 's' && this.look !== 'q') {
          throw new Error(intl.formatMessage({ id: 'ui-inventory-es.parser.strOrQuotedExpression' }));
        }
        ...
        throw new Error(intl.formatMessage({ id: 'ui-inventory-es.parser.invalidSearchClause' }));
      }
    },
    _move() {
      // eat whitespace
